...
namespace Microsoft.OpenApi
{
    ...
    public class OpenApiWorkspace
    {
        ...
        internal OpenApiDocument? FindDocumentByBaseUri(string documentUri)
        {
            ...
            {
                if (doc.BaseUri is not null && doc.BaseUri.ToString().Equals(documentUri, StringComparison.OrdinalIgnoreCase))
                    return doc;
            }
            return null;
        }
        ...
        public static IOpenApiSchema? ResolveDynamicAnchorInContext(IOpenApiSchema? contextSchema, string anchorName)
        {
            ...
            if (contextSchema.DynamicAnchor is string a && a.Equals(anchorName, StringComparison.Ordinal))
